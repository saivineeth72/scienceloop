#!/usr/bin/env python3
"""
Direct test of DatasetAgent (bypassing Flask API)
"""
...
def test_direct():
    """Test DatasetAgent directly"""
    print("=" * 80)
    print("DIRECT TEST OF DATASET AGENT")
    print("=" * 80)
    ...
    print(f"Simulation plan keys: {list(simulation_plan.keys())}")
    
    print("\n" + "=" * 80)
    print("CALLING generate_datasets...")
    print("=" * 80)
    ...
    try:
        ...
        if result.get('datasets'):
            print("\nGenerated Files:")
            for name, path in result['datasets'].items():
                filepath = Path(path)
